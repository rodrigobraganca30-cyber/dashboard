...
<div class="page" id="whatsapp-agenda">
    ...
    <div style="margin-top:24px">
      ...
      <div class="btn-cfg-section" id="btn-cfg-section">
        ...
        <div class="btn-cfg-grid">
          ...
          <div class="btn-cfg-flow-title">🟣 Fluxo D — Recolhimento Template Meta</div>
          <div class="btn-cfg-card">
            <div class="btn-cfg-label green">✅ Botão: "SIM, pode realizar a retirada" → confirmado</div>
            <textarea class="btn-cfg-textarea" id="bcfg-sim-pode-retirada" rows="3"></textarea>
          </div>
          <div class="btn-cfg-card">
            <div class="btn-cfg-label purple">🔄 Botão: "NÃO, preciso reagendar" → reagendado</div>
            <textarea class="btn-cfg-textarea" id="bcfg-nao-preciso-reagendar" rows="3"></textarea>
          </div>
        </div>
  ...
  function waFinishImport(){{
    ...
    for(var i=0;i<s.rows.length;i++){{
      ...
      var _id='wa_'+cleanTel+'_'+nome.replace(/ +/g,'_').slice(0,20);
      var _ofsFromCsv=g(row,m.status)||'—';
      /* Preservar statusOfs original: se já existe no waTratativas, usar o salvo */
      if(!waTratativas[_id])waTratativas[_id]={{}};
      if(!waTratativas[_id].statusOfs)waTratativas[_id].statusOfs=_ofsFromCsv;
      var _ofsFixed=waTratativas[_id].statusOfs;
      list.push({{id:_id,nome:nome,phone:tel,data:g(row,m.data),cidade:g(row,m.cidade),tipo:g(row,m.tipo),endereco:g(row,m.endereco),intervalo:g(row,m.intervalo),statusOfs:_ofsFixed}});
    }}
  ...
  function waSyncStatus(){{
    ...
    fetch(WA_BACKEND+'/all-status?apikey='+(WA_KEY||''),{{headers:h}}).then(function(r){{return r.json()}}).then(function(data){{
      for(var id in data){{
        if(data[id]){{
          var _prevOfs=(waTratativas[id]||{{}}).statusOfs;
          waTratativas[id]=data[id];
          if(_prevOfs&&!waTratativas[id].statusOfs)waTratativas[id].statusOfs=_prevOfs;
        }}
      }}
      waRenderTable();
    }}).catch(function(){{}});
  ...
  (function(){{
    var BCFG_MAP = {{
      ...
      'sim, pode realizar a retirada': 'bcfg-sim-pode-retirada',
      'nao, preciso reagendar':        'bcfg-nao-preciso-reagendar'
    }};
